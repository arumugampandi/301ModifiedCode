...
namespace MT.OnlineRestaurant.SearchManagement.Controllers
{
    ...
    public class SearchController : Controller
    {
        ...
        {
            if (searchDetails.PageSize < 1 && searchDetails.CurrentPage < 1)
            {
                return BadRequest("Page size and Current page should be more than 0");
            }
            ListResponse<RestaurantInformation> restaurantDetails;
            restaurantDetails = business_Repo.SearchForRestaurant(searchDetails);
            return restaurantDetails != null ? this.Ok(restaurantDetails) : this.StatusCode((int)HttpStatusCode.InternalServerError, string.Empty);
        }
        [HttpGet]
        [Route("OrderDetail")]
