...
namespace MT.OnlineRestaurant.DataLayer.Repository
{
    public class SearchRepository : ISearchRepository
    {
            ...
            try
            {
                ...
                foreach (var item in restaurantFilter)
                {
                    RestaurantSearchDetails restaurant = new RestaurantSearchDetails
                    {
                        ...
                        xaxis = item.TblLocation.X != null ? (double)item.TblLocation.X : default,
                        yaxis = item.TblLocation.Y != null ? (double)item.TblLocation.Y : default,
        ...
        private List<RestaurantSearchDetails> GetRetaurantBasedOnLocationAndName(LocationDetails location_Details)
        {
            ...
            try
            {
                ...
                if (!(double.IsNaN(location_Details.xaxis)) && !(double.IsNaN(location_Details.yaxis)))
                {
                    ...
                    {
                        double distance = Distance(location_Details.xaxis, location_Details.yaxis,
                            place.TblLocation.X != null ? (double)place.TblLocation.X : default, place.TblLocation.Y != null ? (double)place.TblLocation.Y : default);
                        if (distance < int.Parse(location_Details.distance.ToString()))
                        {
                            ...
                            RestaurantSearchDetails tblRestaurant = new RestaurantSearchDetails
                            {
                                ...
                                xaxis = place.TblLocation.X != null ? (double)place.TblLocation.X : default,
                                yaxis = place.TblLocation.Y != null ? (double)place.TblLocation.Y : default
                            };
        ...
        private IQueryable<RestaurantSearchDetails> SearchRestaurantSortingBasedOnRating(IQueryable<RestaurantSearchDetails> restaurantSearchDetails, int rating)
        {

            var averageRatingResult = from x in restaurantSearchDetails
                                      join y in db.TblRating on x.restauran_ID equals y.TblRestaurantId into z
                                      from a in z
                                      group a by a.TblRestaurantId into g
                                      select new
                                      {
                                          RestaurantInfo = g.FirstOrDefault(),
                                          RatingAverage = g.Average(x => Convert.ToDouble(x.Rating))
                                      };

            var result = from x in restaurantSearchDetails
                         join rate in averageRatingResult on x.restauran_ID equals rate.RestaurantInfo.TblRestaurantId

                         select new RestaurantSearchDetails
                         {
                             restauran_ID = x.restauran_ID,
                             restaurant_Name = x.restaurant_Name,
                             restaurant_Address = x.restaurant_Address,
                             restaurant_PhoneNumber = x.restaurant_PhoneNumber,
                             restraurant_Website = x.restraurant_Website,
                             closing_Time = x.closing_Time,
                             opening_Time = x.opening_Time,
                             xaxis = x.xaxis,
                             yaxis = x.yaxis,
                             rating = rate.RatingAverage
                         };
            return result.Union(restaurantSearchDetails, new RestaurantSearchDetailsComparer()).Where(x => x.rating >= rating).OrderByDescending(x => x.rating);
        }

        public IQueryable<RestaurantSearchDetails> ApplyPaginationInSearchRestaurant(
            IQueryable<RestaurantSearchDetails> restaurantSearchDetails, Pagination pagination)
        {

            return restaurantSearchDetails.Skip((pagination.CurrentPage - 1) * pagination.PageSize).Take(pagination.PageSize);
